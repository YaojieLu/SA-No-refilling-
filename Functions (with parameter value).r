...
optwLf <- Vectorize(function(wLr){
  ...
  message(wLr, " ", optwLi$maximum, " ", optwLi$objective)
  return(res)
})
...
averAf <- function(wL,
  ...
  integralrLf <- Vectorize(function(w)integrate(rLf, w, 1, rel.tol=.Machine$double.eps^0.5)$value)
  fnoc <- function(w)1/Lf(w)*exp(-gamma*w-k*integralrLf(w))
  cPDF <- 1/integrate(fnoc, wL, 1, rel.tol=.Machine$double.eps^0.5)$value
  
  f1 <- Vectorize(function(w)Af(gsf(w))*cPDF*fnoc(w))
